...
(function ($, document) {
  $.glisse = function (element, options) {
    ...
    plugin.init = function () {
      ...
      plugin.settings.mobile = !!navigator.userAgent.match(
        /iPhone|iPod|iPad|Android/i
      );
    ...
    var createElements = function createElements() {
      ...
      plugin.els["wrapper"] = $(document.createElement("div")).attr(
        "id",
        "glisse-wrapper"
      );
      plugin.els["overlay"] = $(document.createElement("div"))
        .attr("id", "glisse-overlay")
        .css(cssProp, cssVal);
      plugin.els["spinner"] = $(document.createElement("div")).attr(
        "id",
        "glisse-spinner"
      );
      plugin.els["close"] = $(document.createElement("span"))
        .attr("id", "glisse-close")
        .css(cssProp, cssVal);
      plugin.els["content"] = $(document.createElement("div"))
        .attr("id", "glisse-overlay-content")
        .css(cssProp, cssVal)
        .css(getPrefix("transform") + "transform", "scale(0)");
      plugin.els["controls"] = $(document.createElement("div"))
        .attr("id", "glisse-controls")
        .css(cssProp, cssVal);
      plugin.els["controlNext"] = $(document.createElement("span"))
        .attr("class", "glisse-next")
        .append($(document.createElement("a")).html("&#62;").attr("href", "#"));
      plugin.els["controlLegend"] = $(document.createElement("span")).attr(
        "class",
        "glisse-legend"
      );
      plugin.els["controlPrev"] = $(document.createElement("span"))
        .attr("class", "glisse-prev")
        .append($(document.createElement("a")).html("&#60;").attr("href", "#"));
      ...
      plugin.els["controls"].delegate("a", "click", function (e) {
        e.preventDefault();
        var changeTo = $(this).parent().hasClass("glisse-next")
          ? "next"
          : "prev";
    ...
    var changePicture = function changePicture(direction) {
      ...
      if (
        (currentId === 0 && direction === "prev") ||
        (currentId === totGroup - 1 && direction === "next")
      ) {
        change = false;
      }
      ...
      if (change && isChange === false) {
        ...
        var $next =
          direction === "next"
            ? $("img[rel=" + group + "]").eq(currentId + 1)
            : $("img[rel=" + group + "]").eq(currentId - 1);
        if (plugin.settings.mobile) {
          if (direction !== "next") {
            plugin.els["content"].css(
              getPrefix("transform") + "transform",
              "translateX(2000px)"
            );
          } else {
            plugin.els["content"].css(
        ...
        if ($.inArray(pictureUrl, cache) === -1) spinner(true);
        ...
        setTimeout(function () {
          if (plugin.settings.mobile) {
            plugin.els["content"]
              .css(getPrefix("transform") + "transform", "translateX(0px)")
              .css("display", "none");
          }

          var img = $("<img/>", { src: pictureUrl }).appendTo(
            plugin.els["content"]
          );
          plugin.els["content"].css({
          ...
          img.load(function () {
            ...
            if ($.inArray(pictureUrl, cache) === -1) spinner(false);

            if (plugin.settings.mobile) {
              plugin.els["content"].css("display", "block");
            }
            plugin.els["content"]
              .removeClass("glisse-transitionOut-" + direction)
              .addClass("glisse-transitionIn-" + direction);
            setTimeout(function () {
              plugin.els["content"]
                .removeClass("glisse-transitionIn-" + direction)
                .css(cssProp, cssVal);
              isChange = false;
    ...
    var setChangeStyle = function setChangeStyle() {
      ...
      if (!document.getElementById("glisse-css")) {
        $(
          '<style type="text/css" id="glisse-css">' +
            effect +
            changeClass +
            "</style>"
        ).appendTo("head");
    ...
    var setChangeStatus = function setChangeStatus() {
      var $currentEl = $(
          "img[" + plugin.settings.dataName + '="' + pictureUrl + '"]'
        ),
